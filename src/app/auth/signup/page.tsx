  ...
  if (done) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top_left,#dbeafe_0,#f6f8fb_38%,#eef2ff_100%)] px-4 py-10">
        <div className="w-full max-w-md rounded-[2rem] border border-white bg-white/90 p-10 text-center shadow-2xl shadow-slate-200 backdrop-blur">
          <div className="mx-auto grid h-16 w-16 place-items-center rounded-3xl bg-emerald-50 text-3xl">✉️</div>
          <h2 className="mt-5 text-2xl font-black text-slate-950">確認メールを送信しました</h2>
          <p className="mt-3 text-sm font-medium leading-7 text-slate-600">
            <strong>{email}</strong> に確認メールを送りました。
            メール内のリンクから登録を完了してください。
          </p>
          <Link href="/auth/login" className="mt-6 inline-block font-black text-indigo-600 hover:underline">
            ログインページへ
          </Link>
        </div>
      </div>
    ...
    <div className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top_left,#dbeafe_0,#f6f8fb_38%,#eef2ff_100%)] px-4 py-10">
      <div className="w-full max-w-md rounded-[2rem] border border-white bg-white/90 p-8 shadow-2xl shadow-slate-200 backdrop-blur">
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-4 text-2xl font-black text-slate-950">新規登録</h1>
          <p className="mt-1 text-sm font-bold text-slate-500">中1範囲の復習を始めましょう。</p>
        </div>
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <div>
            <label className="mb-1 block text-sm font-bold text-slate-700">パスワード（6文字以上）</label>
            <input
              type="password"
              value={password}
